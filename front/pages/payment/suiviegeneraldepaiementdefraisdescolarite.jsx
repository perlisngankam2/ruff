...
const SuiviPaiementParEleveReNormal = () => {
  ...
  const { data: dataTotalGeneralClassStatisticsFrancophoneSection } = useQuery(
    GET_TOTAL_GENERAL_FEES_STATISTICS_PER_CLASS_FRANCOPHONE_SECTION
  );
  const { data: dataTotalGeneralClassStatisticsAnglophoneSection } = useQuery(
    GET_TOTAL_GENERAL_FEES_STATISTICS_PER_CLASS_ANGLOPHONE_SECTION
  );
            ...
            <TableContainer>
              <Table size="sm">
                ...
                {dataGeneralClassStatisticsFrancophoneSection && (
                  <Tbody>
                    {dataGeneralClassStatisticsFrancophoneSection.getGeneralFrancophoneSectionStatistics.map(
                ...
                {dataTotalGeneralClassStatisticsFrancophoneSection && (
                  <Tbody>
                    {dataTotalGeneralClassStatisticsFrancophoneSection.TotalGeneralFrancophoneSectionStatistics.map(
                      (totalFrancophoneclasse, index) => (
                        <Tr borderBottom={"1px"} key={index}>
                          ...
                          <Td border={"1px"}>
                            {totalFrancophoneclasse.TOTAL_TAUX_ENCAISSEMENT}
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneclasse.TOTAL_RESTE_A_RECOUVRER}
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneclasse.TAUX_RAR}
                          </Td>
                        </Tr>
                      )
                    )}
                ...
                {dataStudentStatisticsFeesPerClassAndSection && (
                  <Tbody>
                    {dataStudentStatisticsFeesPerClassAndSection.getGeneralAnglophoneSectionStatistics.map(
                      (classe, index) => (
                        <Tr border={"1px"} key={index}>
                          ...
                          <Td border={"1px"} borderBottom={"1px"}>
                            {classe.rateZ}%
                          </Td>
                        </Tr>
                      )
                ...
                {dataTotalGeneralClassStatisticsAnglophoneSection && (
                  <Tbody>
                    {dataTotalGeneralClassStatisticsAnglophoneSection.TotalGeneralAnglophoneSectionStatistics.map(
                      (totalAnglophoneclasse, index) => (
                        <Tr key={index}>
                          ...
                          <Td border={"1px"}>
                            {totalAnglophoneclasse.TOTAL_TAUX_ENCAISSEMENT}%
                          </Td>
                          <Td border={"1px"}>
                            {totalAnglophoneclasse.TOTAL_RESTE_A_RECOUVRER}
                          </Td>
                          <Td border={"1px"}>
                            {totalAnglophoneclasse.TAUX_RAR}%
                          </Td>
                        </Tr>
                      )
                    )}
