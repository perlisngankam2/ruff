...
import {
  GET_ALL_PERSONNEL_BY_ID,
  GET_PRIME_PERSONNEL,
  GET_ALL_PAYSALAIRE_BY_ID,
  GET_RETENUE_PERSONNEL,
  GET_Category_Personnel_BY_ID,
  GET_Category_Personnel_ID,
  GET_ALL_SALAIRE_BY_ID,
  GET_ALL_MONTH_SALARY,
  GET_SALARY_NET,
...
const PaySlip = () => {
  ...
  const { data: dataGenererSalaire, refetch } = useQuery(
    GET_ALL_PAYSALAIRE_BY_ID,
    {
      variables: { personnelid: router.query.id },
  ...
  const dernierIndiceGenererSalaire =
    dataGenererSalaire?.getpaysalairebypersonnel.length - 1;
  const dernierElementGenererSalaire =
    dataGenererSalaire?.getpaysalairebypersonnel[dernierIndiceGenererSalaire];
      ...
      {!loading && (
        <DefaultLayout>
          ...
          >
            ...
            >
              <Center w="full">
                {dataPersonnelId?.findOnePersonnel.sexe.toLowerCase() ===
                "homme" ? (
                  <Avatar
                    size="xl"
                    mt={["10px", "10px", "10px"]}
              ...
              >
                {dataPersonnelId?.findOnePersonnel.firstName
                  .charAt(0)
                  .toUpperCase() +
                  dataPersonnelId?.findOnePersonnel.firstName.substring(1) +
                  " " +
                  dataPersonnelId?.findOnePersonnel.lastName
                    .charAt(0)
                    .toUpperCase() +
                  dataPersonnelId?.findOnePersonnel.lastName.substring(1)}
              </Heading>
                ...
                <AlertDialogOverlay>
                  <AlertDialogContent>
                        ...
                        >
                          ...
                          >
                            {dataPersonnelId?.findOnePersonnel.firstName
                              .charAt(0)
                              .toUpperCase() +
                              dataPersonnelId?.findOnePersonnel.firstName.substring(
                                1
                              ) +
                              " " +
                              dataPersonnelId?.findOnePersonnel.lastName
                                .charAt(0)
                                .toUpperCase() +
                              dataPersonnelId?.findOnePersonnel.lastName.substring(
                                1
                              )}
                          </Heading>
                          <Center>
                            <Box mt="20px">
                              ...
                              <Flex gap={6}>
                                ...
                                {dernierElementGenererSalaire && (
                                  <Text fontWeight={"bold"}>
                                    {dernierElementGenererSalaire.moisPaie}
                                  </Text>
                                )}
                              </Flex>
                              <Flex gap={6}>
                                ...
                                <Text fontWeight={"bold"}>
                                  {
                                    dataCategorie?.findOneCategoriepersonnel
                                      .montant
                                  }
                                </Text>
                              </Flex>
                              <Box>
                                <Text>PRIMES SALARIALES</Text>
                                {dataPrimePersonnel ? (
                                  dataPrimePersonnel?.primesETnomprimepersonnel.map(
                                    (prime) => <Flex>{prime[1]}</Flex>
                                  )
                                ) : (
                              ...
                              <Flex gap={6}>
                                ...
                                <Text fontWeight={"bold"}>
                                  {dernierElementGenererSalaire?.montant}
                                </Text>
                              </Flex>
                              <Box
                                width={"340px"}
                              ...
                              >
                                ...
                                <Input
                                  ...
                                  // mt={'8px'}
                                  onChange={(event) =>
                                    setJourPaie(event.target.value)
                                  }
                                  value={jourPaie}
                                  isRequired
                                />
                    ...
                    <AlertDialogFooter>
                      <Button
                        onClick={() =>
                          removePaySalaire(dernierElementGenererSalaire.id)
                        }
                        colorScheme="red"
                      >
                        annuler
