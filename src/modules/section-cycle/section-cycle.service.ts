...
export class SectionCycleService {
    constructor(
      ...
      async create(
        ...
        const cycle = this.cycle.findById(input.cycle_id)
        if(!cycle){
          throw Error("cycle not found")
        }

        const section = this.sectionService.findById(input.section_id)
        if(!section){
      ...
      async update(id:string, input: SectionCycleUpdateInput): Promise<SectionCycle> {
        ...
        if(input.cycle){
            const cycle =
            input.cycle &&
              (await this.cycle.findById(input.cycle_id));
        ...
        if(input.section){
            ...
            this.cycle.update(section.id, input.cycle); 
        }
        wrap(sectionCycle).assign({
            description: input.nom || sectionCycle.description
        },
        { em: this.em },
    );
        await this.sectionCycleRepository.persistAndFlush(sectionCycle);
