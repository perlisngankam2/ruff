...
export class PensionService {
  ...
  async savePension(studentid: string) {
    try {
      const tranchestudent = await this.trancheStudentservice.findByStudents(
        studentid,
      );
      console.log('========>' + tranchestudent);
      const montantpension = tranchestudent
        .map((a) => a.montant)
        .reduce(function (a, b) {
          return a + b;
        });
      const pension = await this.findpensionbystudent(studentid);
      const fees_to_be_paied = await this.studentservice.getclassfeebystudent(
        studentid,
      );
