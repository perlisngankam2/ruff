...
const SuiviePaiementSectionFrancophone = () => {
  ...
  const {
    data: dataStudentStatisticsFrancophoneSection,
    loading,
    error,
  } = useQuery(GET_STUDENT_STATISTICS_FRANCOPHONE_SECTION);
  const { data: dataTotalStudentStatisticsFrancophoneSection } = useQuery(
    GET_TOTAL_STUDENT_STATISTICS_FRANCOPHONE_SECTION
  );
          ...
          <Box display={"flex"} ml={"50px"}>
            <Box mt={"30px"}>
              <Heading textAlign={"center"} fontSize={["sm", "sm", "sm"]}>
                ...
                CLASSE: CM2/ SECTION FRANCOPHONE
              </Heading>
            </Box>
            <Box mt={"30px"} ml={"40px"}>
            ...
            <TableContainer>
              <Table size="sm">
                ...
                {dataStudentStatisticsFrancophoneSection && (
                  <Tbody>
                    {dataStudentStatisticsFrancophoneSection?.getStudentStatisticsFrancophone.map(
                      (francophoneStudent, index) => (
                        <Tr borderBottom={"1px"} key={index}>
                          <Td borderBottom={"1px"}>1</Td>
                          <Td borderBottom={"1px"}>
                            {francophoneStudent.name}
                          </Td>
                ...
                {dataTotalStudentStatisticsFrancophoneSection && (
                  <Tbody>
                    {dataTotalStudentStatisticsFrancophoneSection?.getTotalStudentStatisticsFrancophone.map(
                      (totalFrancophoneStudent, index) => (
                        <Tr borderBottom={"1px"} key={index}>
