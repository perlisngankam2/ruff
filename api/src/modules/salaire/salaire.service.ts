...
export class SalaireService {
  ...
  async create(input: SalaireCreateInput): Promise<Salaire> {
    ...
    if (personnel) {
      ...
      const paysalary = allPaySalary
        .filter(async (a) => (await a.personnel.load()).id == personnel.id)
        .filter((a) => a.moisPaie == input.moisPaie)[0];

      if (!paysalary) {
        throw error(
          "!!!!!!!!!!!!!!!IL N'EXISTE AUCUN PAIEMENT FAIT POUR CE PERSONNEL!!!!!!!!!!!!!!!!",
        );
      }
      ...
      if (
        (await this.getAll())
          .filter(async (a) => a.personnel.id === personnel.id)
          .filter((a) => a.moisPaie == input.moisPaie).length > 1
      ) {
        throw Error(
          '!!!!!!!!!!! LE PAIEMENT DE CE PERSONNEL A DEJA ETE ENREGISTRER POUR CE MOIS !!!!!!!!!!!!',
        );
      }
  ...
  async salairepersonnel(personnelid: string) {
    ...
    if ((await a).length == 0) {
      throw Error(
        'THIS PERSONNEL HAS NOT YET BEING PAIED PLEASE ENTER A VALID PERSONNEL',
      );
    }
    return a;
  }
