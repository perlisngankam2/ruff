...
export class SalleService {
  ...
  async create(input: SalleCreateInput): Promise<Salle> {
...
>>>>>>> e6ca72e1c418a49f9188a391cdc55b4d1fe46cd8
  ...
  async salleAnglophoneSection() {
    ...
    return (await salles).filter(
      (a) =>
        a.niveau.getEntity().cycle.getEntity().section.getEntity().name ===
        'Anglophone',
    );
  }
    ...
    wrap(salle).assign(
      {
        ...
        montantPensionSalle:
          input.montantPensionSalle || salle.montantPensionSalle,
  ...
  async listeInscriptionComplet(id: string) {
    ...
    const liste = [];
    const listeInscrit = await (
      await fraisInscription
  ...
  async findSectionByStudents(studentid: string) {
    const a = (
      ...
      (await (await a.niveau.load()).cycle.load()).section.load(),
    );
