...
const SuiviPaiementFraisInscription = () => {
  ...
  const { data: dataInscriptionStatisticsFrancophoneSection } = useQuery(
    GET_STATISTICS_PRIMAIRE_FRANCOPHONE_CLASS
  );
  const { data: dataTotalInscriptionStatisticsFrancophoneSection } = useQuery(
    GET_TOTAL_STATISTICS_PRIMAIRE_FRANCOPHONE_CLASS
  );
            ...
            <TableContainer>
              <Table size="sm">
                    ...
                    {dataInscriptionStatisticsFrancophoneSection.getSectionStatisticsFrancophoneAdmissionFee.map(
                      (francophoneClass) => (
                        <Tr>
                          ...
                          <Td border={"1px"}>
                            {francophoneClass.numberOfStudents}
                          </Td>
                          <Td border={"1px"}>
                            {francophoneClass.expectedAmount}
                          </Td>
                          <Td border={"1px"}>
                            {francophoneClass.numberOfStudentsStartedPaying}{" "}
                          </Td>
                          <Td border={"1px"}>{francophoneClass.TAUXA}%</Td>
                          <Td border={"1px"}>
                            {francophoneClass.sumAmountAlreadyPaid}
                          </Td>
                          <Td border={"1px"}>{francophoneClass.TAUXB}%</Td>
                          <Td border={"1px"}>
                            {francophoneClass.numberOfStudentsNotPaid}
                          </Td>
                ...
                {dataTotalInscriptionStatisticsFrancophoneSection && (
                  <Tbody>
                    {dataTotalInscriptionStatisticsFrancophoneSection.TotalSectionStatisticsFrancophoneAdmissionFee.map(
                      (totalFrancophoneClass, index) => (
                        <Tr key={index}>
                          <Td border={"1px"} rowSpan={6} fontWeight={"bold"}>
                            Total
                          </Td>
                          <Td border={"1px"}></Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_EFFECTIFS_ENREGISTRES}
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_MONTANT_ATTENDU}
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_NOMBRE_ENCAISSEMENT}
                          </Td>
                          <Td border={"1px"}>
                            {
                              totalFrancophoneClass.TOTAL_FIRST_TAUX_ENCAISSEMENT
                            }
                            %
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_MONTANT_EN_CAISSE}
                          </Td>
                          <Td border={"1px"}>
                            {
                              totalFrancophoneClass.TOTAL_SECOND_TAUX_ENCAISSEMENT
                            }
                            %
                          </Td>
                          <Td border={"1px"}>
                            {
                              totalFrancophoneClass.TOTAL_NOMBRE_SANS_ENCAISSEMENT
                            }
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_FIRST_TAUX_RECOUVRIR}%
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_RESTE_A_RECOUVRER}
                          </Td>
                          <Td border={"1px"}>
                            {totalFrancophoneClass.TOTAL_SECOND_TAUX_RECOUVRIR}%
                          </Td>
                        </Tr>
                      )
                    )}
                ...
                {dataInscriptionStatisticsAnglophoneSection && (
                  <Tbody>
                    {dataInscriptionStatisticsAnglophoneSection?.getSectionStatisticsAnglophoneAdmissionFee.map(
                ...
                {dataTotalInscriptionStatisticsAnglophoneSection && (
                  <Tbody>
                    {dataTotalInscriptionStatisticsAnglophoneSection.TotalSectionStatisticsAnglophoneAdmissionFee.map(
